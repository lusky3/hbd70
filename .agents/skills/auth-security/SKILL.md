...
<!-- Framework: Session-based / HTTP-only Cookies | Generated: 2026-09-19 -->
...
> **Customized for HBD70**: Session-based cookie auth with Fastify `@fastify/cookie` and `@fastify/session`.

### Cookie & Session Rules
- **Cookie flags**: `httpOnly: true`, `secure: true` (in production), `sameSite: 'lax'`
- **Cookie name**: `hbd70_session`
- **Secret rotation**: Signed cookies using `SESSION_SECRET` from environment variables
- **Session state**: Player ID, guest nickname, game progress, and timestamp stored server-side
