...
<!-- Framework: SQLite + Drizzle ORM | Generated: 2026-09-19 -->
...
> **Customized for HBD70**: SQLite database with Drizzle ORM (or better-sqlite3).

### Table Design
- Every table MUST have: `id` (integer primary key or text uuid), `created_at`, `updated_at`
- SQLite file storage: Store local DB file in `server/data/hbd70.db` (gitignored)
- Table names: plural, snake_case (e.g., `players`, `game_sessions`, `scores`)
- Column names: snake_case (e.g., `session_id`, `high_score`, `created_at`)
- Use Drizzle Kit for schema declaration in `server/src/db/schema.ts` and automated migrations.
